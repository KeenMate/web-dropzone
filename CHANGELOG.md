...
- **Built on `@keenmate/web-components-core` (`BlissElement`).** `<web-dropzone>` now extends core's `BlissElement`: the hand-rolled `ATTRIBUTE_TABLE`, `parseAttrValue`, `observedAttributes`, `attributeChangedCallback`, the pre-upgrade property rescue, and ~40 property/callback accessors collapse into one `static inputs` table (`web-component.ts` 1107 → 649 LOC). Parsing, validation, reactivity coalescing, reflection, and form association now come from core. The public attributes, events, and imperative method API are unchanged; three behaviours shift, as in the sibling `web-multiselect` / `web-daterangepicker` migrations:
  - **Property writes coalesce on a microtask.** Setting a property (e.g. `el.maxFileCount = 3`) applies on a microtask — `await el.whenSettled()` before reading derived state back (the imperative methods flush internally, so `el.prop = x; el.method()` still works).
  - **`el.setAttributes({ … })` takes typed values by camelCase config key**, and flushes synchronously (as does `el.batch(fn)`).
  - **Optional-string getters return `null` when unset** (e.g. `el.icon`, `el.promptText`) instead of `''`.
- **Form association via core's `el.form`.** The element drops its own `attachInternals()` for core's lazily-attached `internals` + public `el.form` getter, so host frameworks that delegate by reading `target.form` (e.g. Phoenix LiveView `phx-change`) resolve the owning `<form>` again. Files still submit as real `FormData` blobs via `setFormValue`.
- **Compact-mode popover positioned via core's `/positioning`.** The popover now uses core's `anchor()` (offset → flip → shift + `autoUpdate`, with `maxHeight`/`maxWidth` viewport caps) instead of a direct `@floating-ui/dom` dependency; the renderer no longer declares `@floating-ui/dom` (it resolves transitively through core). Requires `@keenmate/web-components-core` ≥ 1.0.0-rc03 (for the new `anchor({ maxWidth })` option).
- **Categorized logging via core's `/logging`.** `@keenmate/web-dropzone-core`'s loggers (`DROPZONE:INIT/FILE/UI/INTERACTION`) are now built on core's `createLoggers()`; the vendored `loglevel` + prefix-plugin copy and its `%c`-ordering workaround are removed. The logging API (`initLogger`/`fileLogger`/…, `LOGGING_CATEGORIES`, `enable`/`disable`/`setLogLevel`/`setCategoryLevel`) is unchanged. **Consequence:** `@keenmate/web-dropzone-core` gains one runtime dependency, `@keenmate/web-components-core` — it is no longer zero-dependency (correcting the split-out note below).
- **Satellites unchanged.** The four satellites (`<web-dropzone-picker/list/indicator/progress>`) stay on plain `HTMLElement` — they are config-light (a `for=` id + a couple of enum attributes + store-event subscription), so the input-table model buys them little.

### Breaking Changes

- **BlissFramework component guidelines alignment** — the renderer package was brought in line with `BlissFramework/guidelines/web-components/`. Several public theming hooks were renamed or removed:
  - **`--base-*` taxonomy migration to canonical names.** The internal `--dz-*` declarations now read the canonical cross-component vocabulary documented in `guidelines/web-components/base-variables.md`. Consumers who set the previous non-canonical names will see fallbacks kick in:
    - `--base-input-background` → **`--base-main-bg`**
    - `--base-primary-bg` → **`--base-elevated-bg`**
    - `--base-primary-bg-hover` → **`--base-hover-bg`** (now an adaptive `color-mix` chain — derived from text + main-bg if not overridden)
    - `--base-text-on-accent` → **`--base-text-color-on-accent`**
    - `--base-accent-color-light-hover` removed; now derived from `--dz-accent-color` via `color-mix`
    - `--base-text-color-4`, `--base-font-size-lg`, `--base-font-weight-medium`, `--base-line-height-tight` removed — these were component-only extensions never part of the canonical taxonomy. The corresponding `--dz-*` locals still exist and remain themable, just no longer read a `--base-*` lookup.
  - **CSS file renames.** Every `_*.css` partial dropped its underscore prefix (SASS convention has no meaning in plain CSS). Consumers who imported `@keenmate/web-dropzone/src/css/_variables.css` etc. must drop the underscore. The compiled bundle path (`@keenmate/web-dropzone/style.css`) is unchanged.
  - **`@layer variables, component, overrides` cascade.** `main.css` now declares the three-layer cascade. Consumers' unlayered overrides still win — *but* a consumer-side global universal reset (Bootstrap reboot, Tailwind preflight, hand-rolled `* { margin: 0 }`) is itself unlayered and will beat the component's `@layer component` rules. README's new Theming section spells out the wrap-in-a-layer fix.

### Added

- **Dark-mode support** — `light-dark()` fallbacks throughout `variables.css` plus a four-signal `dark-mode.css`: `@media (prefers-color-scheme: dark)`, `:host-context()` framework-class hooks (`data-theme`, `data-bs-theme`, `.dark` / `.light`), and per-instance `:host([data-theme="dark"|"light"])`. Symmetric light overrides included so a dark page can force one instance back to light. No `color-scheme` declaration on `:host` (that would block page inheritance — see `guidelines/web-components/color-scheme.md`).
- **`component-variables.manifest.json`** — at the package root and exported as `@keenmate/web-dropzone/manifest`. Enumerates the 25 canonical `--base-*` variables consumed and the 257 `--dz-*` variables exposed (categorized by feature: accent / text / surface / dropzone / file-list / file-item / badges / popover / preview-grid / progress / rolling / overlay / …). Theme designers and tooling can read it for discoverability.
- **`:host` theme-container completion** — added `position: relative` (offsetParent for absolutely-positioned descendants), `box-sizing: border-box`, and an explicit default `color: var(--dz-text-color-1)` on `:host`. Default `background` is intentionally omitted — the host is a frame, not a surface (documented exception to `guidelines/web-components/theme-container.md`).
- **Canonical Tier-2 CSS file set** — `controls.css`, `floating.css`, `states.css`, `animations.css` created (some as stubs with explanatory comments). `_modifiers.css` content split into `states.css` (disabled / focus / file-status tinting), `animations.css` (the `dz-overlay-fade-in` keyframes), and a new feature file `overlay.css` (the drag-overlay `position:fixed` chrome). `_input-dropdown.css` and `_debug.css` — both copy-pasted from web-multiselect, never imported — were deleted.

### Breaking Changes

- **`@keenmate/web-dropzone-core` split out as a separate package** — the repository is now a workspaces monorepo. `@keenmate/web-dropzone-core` (≈53KB minified / ≈16KB gzipped) is the headless store: `DropzoneCore` class, file-validation pipeline, upload worker pool, event substrate, all domain types, and the public `DropzoneStoreAPI` contract. It has no CSS and no custom-element classes (later in this cycle it took one runtime dependency, `@keenmate/web-components-core`, for the shared `/logging` — see Changed above). `@keenmate/web-dropzone` (the renderer package, same name and on-disk size as before) keeps the `<web-dropzone>` element + the four satellites (`<web-dropzone-picker>`, `<web-dropzone-list>`, `<web-dropzone-indicator>`, `<web-dropzone-progress>`) and the CSS bundle, and positions the popover through core's `/positioning`; it now declares `@keenmate/web-dropzone-core: ^1.0.0` as a hard dependency. End-to-end consumers who already do `import '@keenmate/web-dropzone'` see **no API change** — the renderer transparently bundles core into the same single-file `dropzone.js` so installs are still self-contained at runtime. The dependency exists for the TypeScript type re-exports that survive into `dist/index.d.ts`.
  - **For headless consumers** (React / Vue / Lit / Svelte / Solid wrappers that render the file list themselves): depend on `@keenmate/web-dropzone-core` directly, instantiate `new DropzoneCore(host, config)` against any HTMLElement, and read state via `DropzoneStoreAPI` (`getFiles()`, `getOverallProgress()`, `addFiles(...)`, `removeFile(...)`, etc.) + subscribe to the DOM CustomEvents listed below. You don't pay the cost of the renderer or Floating UI.
  - **Cross-package contract** — the renderer talks to the core through exactly two surfaces: (1) the `DropzoneStoreAPI` TypeScript interface in `packages/web-dropzone-core/src/store-api.ts` covering read + mutate methods; (2) DOM CustomEvents dispatched with `bubbles: true, composed: true` so they cross shadow boundaries: `file-added`, `file-removed`, `files-rejected`, `change`, `files-changed`, `file-progress`, `file-status-changed`, `file-updated`, `file-retry`, `file-uploaded`, `file-deleted`, `store-ready`, `overlay-enter`, `overlay-leave`. No private API access either direction — satellites in particular only know about `DropzoneStoreAPI`, never about `WebDropzone` or `DropzoneCore` concretely.
  - **Migration for existing consumers**: drop-in replacement — version `1.0.0` of `@keenmate/web-dropzone` exports the same symbols at the same paths as `0.x`. The only user-visible new wire is the `file-updated` event (see Added) that handlers can now subscribe to. Existing apps that ignore it keep working unchanged.

- **Async user-confirmation gates + removal of vestigial notification callbacks** — two new callbacks let consumers veto adds and removes with a confirmation dialog (or any async check), and seven post-hoc notification callbacks have been dropped because every one of them was strictly redundant with its event equivalent.
  - **Added:** `beforeFilesAddedCallback(files, existingFiles) → boolean | Promise<boolean>` runs ONCE per add batch after the sync validation gates (`accept`, size, count, `validateCallback`). Files don't appear in `dz.files`, don't fire `file-added`, and don't exist anywhere visible until the promise resolves. Resolving `false` emits `files-rejected` with a new `code: 'cancelled'` (distinct from `'size' | 'type' | 'count' | 'duplicate' | 'custom'`) so consumers can distinguish user-cancel from validator-reject.
  - **Added:** `beforeFilesRemovedCallback(files, allFiles) → boolean | Promise<boolean>` runs on user-initiated removals (row X button, popover Clear button). Programmatic `removeFile(id)` and `clear()` calls SKIP the gate by default — pass `{ confirm: true }` to opt in.
  - **Signature change:** `removeFile(id, opts?: { confirm?: boolean }): Promise<void>` and `clear(opts?: { confirm?: boolean }): Promise<void>` now return Promises. `addFiles(...)` also returns `Promise<void>` because `processFiles` is now async to thread the add gate. Existing sync callers that ignore the return value continue to work.
  - **Removed:** `addCallback`, `removeCallback`, `changeCallback`, `rejectCallback`, `retryCallback`, `uploadedCallback`, `deleteCallback`. All were post-hoc notification with a `void` return and zero consumers in the codebase — the matching events (`file-added`, `file-removed`, `change`, `files-rejected`, `file-retry`, `file-uploaded`, `file-deleted`) cover every use case with strictly more flexibility (multi-subscriber). The framework's internal form-value sync that used to ride on `removeCallback` / `changeCallback` wrappers now hooks the host's own composed `change` event listener.
  - **Kept:** `validateCallback` (sync technical-validation gate — different role from the new async gates), `uploadFileCallback`, `retryPolicy`, the render callbacks, `customStylesCallback`, `persistStateCallback` / `loadStateCallback`. These do work events can't.

- **Event detail payloads tightened to a single contract** — the host element is on `e.target`, the full snapshot is on `e.target.files`, and `detail` carries only what can't be recovered from those. Per-file events (`file-added`, `file-removed`, `file-retry`, `file-uploaded`, `file-deleted`) now dispatch with `detail: { file }`. `change` matches the native `<input type="file">` convention with an empty detail — read `e.target.files` for the snapshot. `overlay-enter` / `overlay-leave` drop the redundant `files` field but still carry `overlayElement` (which the consumer can't get otherwise). Listeners that want the rAF-coalesced batch diff should subscribe to `files-changed` (`{ changedIds, files }`); batch events (`files-rejected`, `files-changed`) and substrate events (`file-progress`, `file-status-changed`) were already on this shape and are unchanged. The duplicate `files` field encouraged listeners to mix per-file and snapshot semantics interchangeably and inflated detail payloads on what should be tight dispatches.
...
- **`selector-appearance="native"` — mimics the browser's native `<input type="file">`** — button + plain text label on one inline row. Empty state shows "No file chosen" (or whatever you set via the new `no-file-chosen-text` attribute / `noFileChosenText` property), single-file state shows the filename, multi-file state shows "N files chosen". Reuses `select-files-text` for the button label. Works as a drop-in replacement for native file inputs in tight forms; pairs with `list-appearance="none"` for the truest native look, or with any other list-appearance to show the full file list below the inline summary. The new attribute is i18n-friendly. Wired through both the in-class bulk renderer (`dropzone.ts`) and the satellite picker (`web-component-picker.ts`).
...
- **`file-updated` event** — catch-all post-mutation signal for any `FileState` field other than `progress` / `status` (which have their own dedicated events). Detail is `{ file }`, bubbles + composed. Fires from two places: (1) the preview-generation path when `previewUrl` lands on a file via `FileReader → data URL`; (2) the upload-success path when the handler-returned `FileUploadResult` merges `metadata` / `downloadUrl` / `previewUrl` / `name` onto the file. Internally gated by a `didMerge` flag so no-op writes don't dispatch. Was added as the architectural prep for the package split: with the core / renderer boundary now real, the renderer can't reach into core methods even by inheritance — it subscribes to `file-updated` to know when to re-render a row that's had a server-id stamped on it. BYO renderers (React / Vue / Lit) consume the same event for the same reason. New `FileUpdatedEventDetail` type exported from `@keenmate/web-dropzone-core`.
...
- **Five small TS helpers extracted to dedicated modules** — consolidation pass with no public API changes; all five removals were verified-identical duplicates.
  ...
  - **`createMicrotaskScheduler(fn)`** in `satellite-base.ts` — three satellites (`list`, `indicator`, `progress`) previously had identical `private scheduled = false; queueMicrotask(() => { scheduled = false; fn() })` blocks. The factory returns a closed-over deduped scheduler so each satellite declares one field initializer instead of a flag + method pair.
  - **`resolveEnumAttribute<T>(el, attr, allowed, fallback)`** in `satellite-base.ts` — generic attribute-to-typed-enum reader used by `selector-appearance` / `card-size` / `list-appearance` / `position` / `drawer`. Removes six private `resolve*` methods that all followed the same shape (`getAttribute → check against const tuple → cast or default`).
  - **`warnStoreMissing(elementName, forId)`** in `satellite-base.ts` — standard "store not found" `console.warn` for satellites. Four near-identical warns (with one variation in the indicator's wording) collapsed into one helper with consistent diagnostic text.
  - Net effect: ~140 lines of duplicated boilerplate removed, three new test-able units (`dispatchComposedEvent`, `createMicrotaskScheduler`, `resolveEnumAttribute`) extracted from class internals.
...
- **Multiple `<web-dropzone-picker>`s all highlighted at once during a drag (per-picker highlight was page-global).** The card picker's `dragActive` highlight was driven by **document-level** `dragenter`/`dragleave` events (a shared depth counter wired in `wireDragOverlayTarget`), so any drag anywhere on the page toggled *every* picker's `dz__dropzone--active` together. In a multi-picker layout (e.g. Mode B: Picker A + Picker B side by side) dragging over one bucket did not distinctly highlight it, and moving the cursor between buckets lit up both. The highlight is now scoped to each picker's own surface via an enter/leave counter on its `.dz__dropzone` element; document-level listeners are reduced to a `drop`/`dragend` reset safety net (they no longer *activate* the highlight). `setDragActive` also toggles the live card's class + prompt text in place instead of re-rendering, so the drop target isn't destroyed mid-drag. The `examples-architecture.html` "topology in one diagram" was also cleaned up (2-column grid instead of hand-counted space alignment, which overlapped).
